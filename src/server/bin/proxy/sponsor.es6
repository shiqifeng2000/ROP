...
let express = require('express'), agent = require('../agent'), ropRouter = require('../router'),
  log4js = require('log4js'), constant = require('../constant'), i18n = require('../i18n'), router = express.Router(),
  skipper = (req, res, next) => {next()}
  ...
  agent.deliver(ropRouter.deliverOption('common', 'sponsor', 'getId', param))
    .then(rawBody => {
      ...
      if (((typeof body.is_success == 'boolean') && body.is_success) || ((typeof body.is_success == 'string') && (body.is_success == 'true'))) {
        ...
        res.render(`pages/${agent.mobileSwitch('supplier', req)}/index`, {
          _session_id: req.session.id,
          _info: `window.ssv_info = ${JSON.stringify({ssv_id: body.ssv_user_id, ssv_name: body.ssv_user_name})}`,
          _injection: constant.clientSideScript
        }, (error, html) => {
          agent.renderErrorHandler(res, error, html)
        })
      } else {
        res.redirect(`${constant.protocol}://${constant.domain}`)
      }
    }, why => {
...
catRouter.get('*', function (req, res, next) {
  ...
  agent.deliver(ropRouter.deliverOption('common', 'sponsor', 'get_cat_ssv', param))
    .then(rawBody => {
      ...
      const body = (constant.ROPRequestType == 1) ? rawBody : JSON.parse(rawBody.rop_api_data_get_response.result)
      if (((typeof body.is_success == 'boolean') && body.is_success) || ((typeof body.is_success == 'string') && (body.is_success == 'true'))) {
        res.redirect(`${constant.protocol}://${body.user_domain}.${constant.domain}/supplier/document`)
      } else {
