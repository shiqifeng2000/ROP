...
const _env = require('../key.json').env, _code = require('./error.json'), suffix = `t=${new Date().getTime()}`
...
const clientRequires = {
  ...
  paths: {
    ...
    angularDND: `../../vendor/angular-dragdrop/src/angular-dragdrop.min`,
    ...
    angularEditor: `./components/ckeditor`,
    ...
    inview: `../../vendor/in-view/dist/in-view.min`,
    ...
    jQueryUI: `../plugin/jquery-ui-1.12.1.custom/jquery-ui.min`,
  ...
  shim: {
    ...
    /* angularMaterial: {
     deps: ['angularAria', 'angularMessages', 'angularAnimate']
     },
     angularCookie: {
     deps: ['angular']
     },
...
const constant = {
  pro: {
    ...
    errFile: `${__dirname}/../logs/err.log`,
    errorPath: 'error',
    localePath: 'locales',
    viewsPath: 'views',
    staticPath: 'public',
    ...
    moduleList: ['dashboard', 'application', 'console', 'sso', 'supplier', 'user', 'welcome', 'common'],
    templateList: ['pagination', 'tree_view', 'stepper', 'datatable', 'fixed_table', 'logger'],
    ROPRequestType: 1,
    generateROPStandardRequest (type) {
      return (type == 1) ? '?method=ruixue.rop.frontend.api.data.get&&format=json&&access_token=BCB8D371-EBF1-4549-A272-EAFE7F342DBF' : {
        'method': 'ruixue.rop.api.data.get',
        'format': 'json',
        'access_token': 'BCB8D371-EBF1-4549-A272-EAFE7F342DBF'
      }
    },
  ...
  test: {
    ...
    ROPRequestType: 1,
    generateROPStandardRequest (type) {
      return (type == 1) ? '?method=ruixue.rop.frontend.api.data.get&&format=json&&access_token=933C10E3-DC2B-4514-9F7A-27478D34B3C5' : {
        'method': 'ruixue.rop.api.data.get',
        'format': 'json',
        'access_token': '933C10E3-DC2B-4514-9F7A-27478D34B3C5'
      }
    },
    ...
    simpleApiHost: 'http://115.159.67.217:30005/rop/rest',
    apiTimeout: 49000,
    couchbaseHost: 'http://10.205.16.46:8091/pools',
    couchbaseBucket: 'open.rongcapital.cn',
    couchbasePassword: 'q1w2e3',
    useCluster: false,
    sessionStore: 'couchbase'
