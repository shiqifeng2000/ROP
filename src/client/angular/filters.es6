...
angular.module('rop.filters', [])
  ...
  .filter('hiddenSecret', () => str => {
    var myStr = String(str), tripleLength = Math.floor(myStr.length / 5)
    return myStr.replace(/./g, (match, i) => {
      return (i > tripleLength) && (i < (myStr.length - tripleLength)) ? '*' : match
    })
  })
