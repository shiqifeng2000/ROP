  ...
  CalendarCtrl.prototype.getFocusDateFromKeyEvent = function (event) {
    ...
    switch (event.which) {
      case keyCode.RIGHT_ARROW:
        return dateUtil.incrementDays(this.focusDate, 1)
      case keyCode.LEFT_ARROW:
        return dateUtil.incrementDays(this.focusDate, -1)
      case keyCode.DOWN_ARROW:
        return event.metaKey ? dateUtil.incrementMonths(this.focusDate, 1) : dateUtil.incrementDays(this.focusDate, 7)
      case keyCode.UP_ARROW:
        return event.metaKey ? dateUtil.incrementMonths(this.focusDate, -1) : dateUtil.incrementDays(this.focusDate, -7)
      case keyCode.PAGE_DOWN:
        return dateUtil.incrementMonths(this.focusDate, 1)
      case keyCode.PAGE_UP:
        return dateUtil.incrementMonths(this.focusDate, -1)
      case keyCode.HOME:
        return dateUtil.getFirstDateOfMonth(this.focusDate)
      case keyCode.END:
        return dateUtil.getLastDateOfMonth(this.focusDate)
      default:
        return null
    }
  }
...
(function (angular) {
  ...
  TimeSpinnerCtrl.prototype.handleBodyMouseOut = function (event) {
    var self = this, event = event || window.event
  ...
  TimeSpinnerCtrl.prototype.buildTime = function (hour, minute, second) {
    var myHour = Number.parseInt(hour || this.hour),
      myMinute = Number.parseInt(minute || this.minute),
      mySecond = Number.parseInt(second || this.second)
...
(function (angular) {
      ...
      template: `
                ...
                <div class="shortcut-calendar-body">
                    ...
                    <div ng-if="ctrl.activeTab == 1" class="wrapper">
                        <md-button class="md-raised" ng-class="{'active':ctrl.activeTab == relativeIndex}" aria-label="data control" ng-repeat="relativeIndex in ctrl.relativeIndexs track by $index" ng-click="ctrl.setRangeToRelative(relativeIndex)"><span ng-bind="'前' + (-relativeIndex) + '天'"></span></md-button>
                    </div>
                </div>
  ...
  RopShortcutCalendarCtrl.prototype.setRangeToThisWeek = function () {
    var dayIndex = this.today.getDay(),
      firstDayOfTheWeek = this.dateUtil.incrementDays(this.today, 1 - (dayIndex || 7)),
  ...
  RopShortcutCalendarCtrl.prototype.setRangeToLastSeason = function () {
    var dateInLastSeason = this.dateUtil.incrementMonths(this.today, -3),
      months = this.dateUtil.getMonthsInSeason(dateInLastSeason), monthIndex = dateInLastSeason.getMonth(),
  ...
  RopShortcutCalendarCtrl.prototype.setRangeToRelative = function (days) {
    var targetDay = this.dateUtil.incrementDays(this.today, days),
      yesterday = this.dateUtil.incrementDays(this.today, -1)
  ...
  angular.module('rop.module.calendar').config(['$provide', ($provide) => {
    ...
    DateLocaleProvider.prototype.$get = function ($locale) {
      ...
      function defaultFormatDate (date) {
        ...
        var monthNum = formatDate.getMonth() + 1, dateNum = formatDate.getDate()
        return formatDate.getFullYear() + '-' + `00${monthNum}`.substr(`${monthNum}`.length) + '-' + `00${dateNum}`.substr(`${dateNum}`.length)
      }
...
(function (angular) {
  ...
  angular.module('rop.module.calendar').factory('$$mdDateUtil', function () {
    ...
    function isMinTimeBeforeMaxTime (minTime, maxTime, token) {
      ...
      var minTimeArray = minTime.split(token).map(r => {return Number.parseInt(r)}),
        maxTimeArray = maxTime.split(token).map(r => {return Number.parseInt(r)})
    ...
    function isTimeWithinRange (time, minTime, maxTime, token) {
      ...
      var timeArray = time.split(token).map(r => {return Number.parseInt(r)}),
        minTimeArray = minTime.split(token).map(r => {return Number.parseInt(r)}),
        maxTimeArray = maxTime.split(token).map(r => {return Number.parseInt(r)})
