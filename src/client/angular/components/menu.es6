...
(function( window, angular, undefined ){
  ...
  angular.module('rop.material.components.menu', [
    'material.core',
  ...
  angular
    .module('rop.material.components.menu')
    .controller('ropMenuCtrl', MenuController);

  /**
   * ngInject
   */
  function MenuController($ropMenu, $attrs, $element, $scope, $mdUtil, $timeout, $rootScope, $q) {
...
/*
      this.onOpen = $attrs.onOpen && (typeof $scope[$attrs.onOpen] == "function") ? $scope[$attrs.onOpen] : undefined
      this.onClose = $attrs.onClose && (typeof $scope[$attrs.onClose] == "function") ? $scope[$attrs.onClose] : undefined
*/
    ...
    this.enableHoverListener = function() {
      deregisterScopeListeners.push($rootScope.$on('$ropMenuOpen', function(event, el) {
        if (menuContainer[0].contains(el[0])) {
          self.currentlyOpenMenu = el.controller('ropMenu');
          self.isAlreadyOpening = false;
          self.currentlyOpenMenu.registerContainerProxy(self.triggerContainerProxy.bind(self));
        }
      }));
      deregisterScopeListeners.push($rootScope.$on('$ropMenuClose', function(event, el) {
        if (menuContainer[0].contains(el[0])) {
    ...
    this.handleMenuItemHover = function(event) {
      ...
      openMenuTimeout = $timeout(function() {
        if (nestedMenu) {
          nestedMenu = angular.element(nestedMenu).controller('ropMenu');
        }
    ...
    /**
     * Uses the $ropMenu interim element service to open the menu contents
     */
    this.open = function openMenu(ev) {
      ...
      $scope.$emit('$ropMenuOpen', $element);
      $ropMenu.show({
        scope: $scope,
        ropMenuCtrl: self,
        ...
        parent: 'body',
        onROPMenuShow: $attrs.onOpen && (typeof $scope[$attrs.onOpen] == "function") && $scope[$attrs.onOpen] || undefined,
        onROPMenuRemove: $attrs.onHide && (typeof $scope[$attrs.onHide] == "function") && $scope[$attrs.onHide] || undefined
      }).finally(function() {
        triggerElement.setAttribute('aria-expanded', 'false');
    ...
    $scope.$watch(function() { return self.isOpen; }, function(isOpen) {
      ...
      $scope.$ropMenuIsOpen = self.isOpen;
    });
    ...
    this.destroy = function() {
      return self.isOpen ? $ropMenu.destroy() : $q.when(false);
    };

    // Use the $ropMenu interim element service to close the menu contents
    this.close = function closeMenu(skipFocus, closeOpts) {
      if ( !self.isOpen ) return;
      self.isOpen = false;

      var eventDetails = angular.extend({}, closeOpts, { skipFocus: skipFocus });
      $scope.$emit('$ropMenuClose', $element, eventDetails);
      $ropMenu.hide(null, closeOpts);
  ...
  MenuController.$inject = ["$ropMenu", "$attrs", "$element", "$scope", "$mdUtil", "$timeout", "$rootScope", "$q"];
   ...
   * The trigger element's scope has access to `$ropOpenMenu($event)`
   ...
   *  <md-button ng-click="$ropOpenMenu($event)" class="md-icon-button" aria-label="Open sample menu">
   ...
   * <hljs lang="html">
   * <md-menu>
   *  <md-button ng-click="$ropOpenMenu($event)" class="md-icon-button" aria-label="Open some menu">
   ...
   * You can then close the menu programatically by injecting `$ropMenu` and calling
   * `$ropMenu.hide()`.
   *
   * <hljs lang="html">
   * <md-menu-item>
   ...
   * <hljs lang="html">
   * <md-menu>
   *  <md-button ng-click="$ropOpenMenu($event)" class="md-icon-button">
   *    <md-icon md-svg-icon="call:phone"></md-icon>
   *  </md-button>
   *  <md-menu-content>
   *    <md-menu-item><md-button ng-click="doSomething()">Do Something</md-button></md-menu-item>
  ...
  angular
    .module('rop.material.components.menu')
    .directive('ropMenu', MenuDirective);
  ...
  function MenuDirective($mdUtil) {
    ...
    return {
      restrict: 'E',
      require: ['ropMenu', '?^ropMenuBar'],
      controller: 'ropMenuCtrl', // empty function to be built by link
      scope: true,
      compile: compile
    };
    ...
    function link(scope, element, attrs, ctrls) {
      ...
      ropMenuCtrl.init(menuContainer, { isInMenuBar: isInMenuBar });
    }
  }
  ...
  function MenuProvider($$interimElementProvider) {
    ...
    return $$interimElementProvider('$ropMenu')
      .setDefaults({
        methods: ['target'],
    ...
    function menuDefaultOptions($mdUtil, $mdTheming, $mdConstant, $document, $window, $q, $$rAF, $animateCss, $animate) {
      ...
      function onRemove(scope, element, opts) {
        ...
        return (opts.$destroy === true) ? detachAndClean() : animateRemoval().then( detachAndClean ).finally(()=>{
          opts.scope.keyword = null
          opts.onROPMenuRemove && opts.onROPMenuRemove()
        });
      ...
      function onShow(scope, element, opts) {
        ...
        return showMenu()
          .then(function(response) {
            ...
            opts.onROPMenuShow && opts.onROPMenuShow()
          });
        ...
        function sanitizeAndConfigure() {
          if (!opts.target) {
            throw Error(
              '$ropMenu.show() expected a target to animate from in options.target'
            );
          }
          angular.extend(opts, {
            alreadyOpen: false,
        ...
        function activateInteraction() {
          ...
          if ( !focusTarget ) {
            ...
            focusTarget = firstChild && ((firstChild.querySelector('.md-button:not([disabled])') || firstChild.querySelector('input')) || firstChild.firstElementChild);
          }
          ...
          function onMenuKeyDown(ev) {
            ...
            switch (ev.keyCode) {
              case $mdConstant.KEY_CODE.ESCAPE:
                opts.ropMenuCtrl.close(false, { closeAll: true });
                handled = true;
                break;
              // 李楠需求，在输入时回车要退出
              case $mdConstant.KEY_CODE.ENTER:
                if ($document[0].activeElement.tagName.toLowerCase() == "input") {
                  ev.preventDefault()
                  ev.stopPropagation()
                  if (!focusMenuItem(ev, opts.menuContentEl, opts, 1) && !opts.nestLevel) {
                    opts.ropMenuCtrl.triggerContainerProxy(ev);
                    handled = true
                    break;
                  }
                }
          ...
          function onBackdropClick(e) {
            ...
            scope.$apply(function() {
              opts.ropMenuCtrl.close(true, { closeAll: true });
            });
          }
       ...
       * @param {object} opts - The interim element options for the ropMenu
      ...
      function focusMenuItem(e, menuEl, opts, direction) {
        ...
        for (var i = currentIndex + direction; i >= 0 && i < items.length; i = i + direction) {
          var focusTarget = items[i].querySelector('.md-button') || items[i].querySelector('input');
      ...
      function calculateMenuPosition(el, opts) {
        ...
        var positionMode = opts.ropMenuCtrl.positionMode();
        ...
        var offsets = opts.ropMenuCtrl.offsets();
