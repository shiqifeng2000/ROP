...
consoleApp.config(['$provide', '$logProvider', '$compileProvider', ($provide, $logProvider, $compileProvider) => {
...
consoleApp.controller('ConsoleCtrl', ['$rootScope', '$scope', '$state', '$location', '$window', '$http', '$cookies', '$mdDialog',
  ($rootScope, $scope, $state, $location, $window, $http, $cookies, $mdDialog) => {
    ...
    $rootScope.isAdmin = () => $rootScope.profile && (($rootScope.profile.login_user_type == '0') || ($rootScope.profile.login_user_type == '3'))
    $rootScope.isSsv = () => $rootScope.profile && ($rootScope.profile.login_user_type == '2')
    $rootScope.isIsv = () => $rootScope.profile && ($rootScope.profile.login_user_type == '1')
    ...
    $rootScope.quitSession = (from) => {
      let self = this
      return $http.post('/quit').then(body => {
        if (((typeof body.data.is_success == 'boolean') && body.data.is_success) || ((typeof body.data.is_success == 'string') && (body.data.is_success == 'true'))) {
          $cookies.put('_session', null, {path: '/', domain: `${Constant.nosubdomain ? '' : '.'}${Constant.host}`})
          $cookies.remove('_session', {path: '/', domain: `${Constant.nosubdomain ? '' : '.'}${Constant.host}`})
          $rootScope.toPlatform(`sso${from ? ('?from=' + from) : ''}`)
        } else {
          self.warn(body.msg)
        }
      }, why => {self.alert(why)})
    ...
    $rootScope.getSystemHints = () => {
      ...
      socket.on('message', (message) => {
        let _session = $cookies.get('_session') ? JSON.parse($cookies.get('_session')) : '', quiter = () => {
          window.location.href = `${Constant.protocol}://${Constant.host}:${Constant.port}/sso?from=console`
        }

        if (!_session || !$rootScope.profile || (document.querySelector('meta[name=session]').content != _session.id)) {
          return quiter()
        }
