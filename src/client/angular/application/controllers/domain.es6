    ...
    ($rootScope, $scope, $http, $stateParams, $location, $mdDialog, $q) => {
      ...
      $scope.reset = () => {
        ...
        $scope.columns = [{text: '结构名称', name: 'domain_name', tooltip: true, sort: '1'}, {
          ...
          name: 'domain_title',
          tooltip: true
        }, {text: 'API类型', name: 'cat_name', style: {width: '128px', 'text-align': 'center'}}, {
          text: '状态',
          name: 'status_name',
        ...
        return $scope.ajax('domain_list', {
          ...
          if (((typeof data.is_success == 'boolean') && data.is_success) || ((typeof data.is_success == 'string') && (data.is_success == 'true'))) {
            ...
            if ($scope.tableData && $scope.tableData[0]) {
              if (previous) {
                $scope.tableData._select = [].concat.apply([], $scope.tableData.map(r => ((previous.domain_id == r.domain_id) ? [r] : [])))[0] || $scope.tableData[0]
              } else {
      ...
      $scope.canVisistDomain = () => $scope.tableData && $scope.tableData._select
      $scope.canInsertDomain = () => $rootScope.validateBtnRole('btnInsert')
      $scope.canUpdateDomain = (domain) => {
        if (!domain || (domain.is_old == '1')) {return false}
        // 流程已变化，任何状态下都可修改
        return true
      ...
      $scope.canRecallDomain = (domain) => {
        if (!domain || (domain.cancel_flag == '0') || (domain.is_old == '1')) { return false}
      ...
      $scope.deleteDomain = ($event, row) => {
              ...
              template: `<md-dialog aria-label="Mango (Fruit)" style="width: 512px">
                                <md-dialog-content class="md-dialog-content">
                                    <div class="md-dialog-content-body">
                                        <p>操作失败</p>
                                        <p ng-bind="hint"></p>
                                        <p ng-repeat="api in apis track by $index" ng-bind="api.name"></p>
                                        <p>请取消关联后再删除</p>
                                    </div>
                                </md-dialog-content>
        ...
        return $rootScope.confirm('请确认是否要删除结构？', () => {
          return $scope.ajax('domain_delete', {domain_id: row.domain_id}, (data) => {
            $scope.research()
      ...
      $scope.applyDomain = (row, status) => {
        ...
        return $scope.ajax('domain_apply', {domain_id: row.domain_id, apply_status: status}, (data) => {
          $scope.searcher()
      ...
      $scope.cancelDomain = (row) => {
        return $scope.ajax('domain_cancel', {domain_id: row.domain_id}, (data) => {
          $scope.searcher()
          $rootScope.warn('撤销成功', 1)
        })
      }
      /*$scope.dumpDomain = ()=> {
       return $scope.ajax("domain_dump", {api_id: row.domain_id}, (data)=> {
       $scope.research();
      ...
      let initialUpdaterDomain = {
          ...
          domain_desc_en: '',
          property: [{}, {}, {}, {}, {}]
        }, initialSteps = [{name: '基本信息'}, {name: '属性信息'}], initialClientSteps = [{name: '基本信息'}, {name: '错误代码'}],
        initDomainQ
      ...
      $scope.initDomain = (row) => {
        return $scope.ajax('domain_init', {}, (data) => {
      ...
      $scope.addDomainMode = (mode) => {
        initDomainQ && initDomainQ.then(data => {
          ...
          $scope.steps = initialSteps
          $scope.ajax('domain_datatype', {cat_id: $scope.updaterSelection.panelCat.cat_id}, (data) => {
      ...
      $scope.updateDomainMode = ($event, forView) => {
        ...
        let updater = () => {
          return initDomainQ && initDomainQ.then(data => {
              ...
              return $scope.ajax('domain_info', {domain_id: $scope.tableData._select.domain_id}, (data) => {
                $scope.mode = 1
                $scope.steps = initialSteps;
                ($scope.step != 0) && ($scope.step = 0)
          ...
          scope.confirmUpdate = () => {
            scope.loading = true
            // 后台更新和详情获取可以同时进行
            $scope.ajax('domain_status_apply', {domain_id: $scope.tableData._select.domain_id}, updater).then(() => {$scope.searcher()}).then(() => {
              scope.loading = false
              $mdDialog.hide()
            })
          }
          ...
          $mdDialog.show({
            ...
            template: `<md-dialog aria-label="Mango (Fruit)" style="width: 512px">
                          <md-dialog-content class="md-dialog-content">
                              <div class="md-dialog-content-body">
                                  <md-progress-circular md-mode="indeterminate" ng-show="loading" class="md-primary" style="position:absolute;top: 16px;left: 16px;"></md-progress-circular>
                                  <p ng-bind="hint"></p>
                                  <p ng-repeat="api in apis track by $index" ng-bind="api.name"></p>
                                  <p>您确认要进行修改吗</p>
                              </div>
      ...
      $scope.nextStep = () => {
        if ($scope.updaterDomain._forView) {
          ($scope.step < 3) && $scope.step++
          return
        }
        if ($scope.updaterDomainForm.$invalid) {
          $scope.updaterDomainForm.$error.required && $scope.updaterDomainForm.$error.required.forEach(r => {r.$setDirty(true)})
          return
        }
      ...
      $scope.saveDomain = () => {
        ...
        let domain = angular.extend({
          cat_id: $scope.updaterSelection.panelCat.cat_id,
          domain_property: [].concat.apply([], $scope.updaterDomain.property.map(r => {return r.pro_name ? [r] : []}))
        }, $scope.updaterDomain)
        ...
        return $rootScope.confirm('系统将保存结构设置，您确认要保存吗', () => {
          if ($scope.refreshing) {return}
          $scope.refreshing = true
          return $scope.ajax('domain_save', domain).then((data) => {
            $scope.refreshing = false
            $scope.resetOrder()
            $scope.exitMode()
            $scope.step = 0
      ...
      $scope.sortSave = () => {
        ...
        return $scope.ajax('sort_save', {domain_list: domainIDs}, (data) => {
          $scope.enterMode(0)
        })
      }
      $scope.resetOrder = () => {
