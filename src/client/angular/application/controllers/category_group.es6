...
define([], function (cta) {
  ...
  return ['$rootScope', '$scope', '$q', '$http', '$timeout', '$mdDialog','$location',
    ($rootScope, $scope, $q, $http, $timeout, $mdDialog,$location) => {
      ...
      function initFun(){
        return $scope.ajax('cat_group_init').then(data => {
      ...
      function initApplyFun(){
        return $scope.ajax('cat_group_apply_init').then(data => {
      ...
      $scope.searcher = (index, size) => {
        ...
        return initQ.then(() => {
          return $scope.ajax('cat_group_list', {
      ...
      $scope.canCreateCategory = () => $rootScope.validateBtnRole('btnInsert')
      $scope.canUpdateCategory = () => $scope.tableData._select && $rootScope.validateBtnRole('btnUpdate')
      $scope.canDeleteCategory = () => $scope.tableData._select && $rootScope.validateBtnRole('btnDelete')
      $scope.canManageCategory = () => $rootScope.validateBtnRole('btnApply')
      ...
      $scope.deleteCat = (group) => {
        return $rootScope.confirm('请确认是否要删除子分类？', () => {
          if ($scope.refreshing) {return}
          $scope.refreshing = true
          return $scope.ajax('cat_group_del', {group_id: group.group_id}, (data) => {
            $scope.research()
      ...
      let getCheckedQ = () => $scope.ajax('cat_group_checked', {
        cat_id: $scope.currentApplyCat.cat_id
      }).then(data => {
        $scope.check_list = data.data_list
        return data
      }), getUncheckedQ = () => $scope.ajax('cat_group_unchecked', {
        cat_id: $scope.currentApplyCat.cat_id
      }).then(data => {
      ...
      $scope.manageCat = () => {
        ...
        initApplyQ.then(() => {
          $scope.mode = 1
          checkQ = getCheckedQ().then(data => {
            $scope.audit_list = $scope.check_list
            return data
          })
          uncheckQ = getUncheckedQ().then(data => {
            $scope.tobe_audit_list = $scope.uncheck_list
            return data
          })
      ...
      $scope.applyCat = () => {
        ...
        if (!list.length) {
          $rootScope.alert('请选择一条子分类')
          return
        }

        if ($scope.refreshing) {return}
        $scope.refreshing = true
        return $scope.ajax('cat_group_apply', {
          group_list: list,
          cat_id: $scope.currentApplyCat.cat_id
        }).then(() => {
          checkQ = getCheckedQ().then(data => {
      ...
      $scope.showCat = (ev, row) => {
        ...
        $scope.refreshing = true
        return $scope.ajax('save_group_init', {group_id: row ? row.group_id : undefined}).then(data => {
                                    ...
                                    <form class="md-dialog-content" name="catGroupForm" ng-submit="saveMail()" autocomplete="off">
                                        <div class="input-container">
                                            ...
                                            <md-menu class="md-dropdown cool" md-offset="1 50" >
                                                <md-button ng-click="$mdOpenMenu($event)" class="md-raised md-primary md-hue-1" md-theme="dracular" aria-label="data control" ng-disabled="detail.group_id" style="max-width: 252px;padding-right: 30px;text-overflow: ellipsis;">
                                                ...
                                                <md-menu-content class="md-dropdown-content cool" style="width: 264px">
                                                    <md-menu-item ng-repeat="cat in cat_list track by $index"><md-button ng-click="chooseType(cat)">{{cat.cat_name}}</md-button></md-menu-item>
                                                </md-menu-content>
                                            </md-menu> 
                                        </div>
                                        <div class="input-container">
