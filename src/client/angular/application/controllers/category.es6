...
define([], function (cta) {
  ...
  return ['$rootScope', '$scope', '$q', '$http', '$timeout', '$mdDialog','$location',
    ($rootScope, $scope, $q, $http, $timeout, $mdDialog,$location) => {
      ...
      $scope.canUpdateCategory = () => $scope.tableData._select
      $scope.canDeleteCategory = () => $scope.tableData._select
      ...
      $scope.deleteCat = (cat) => {
        return $rootScope.confirm('请确认是否要删除分类？', () => {
          if ($scope.refreshing) {return}
          $scope.refreshing = true
          return $scope.ajax('cat_del', {cat_id: cat.cat_id}, (data) => {
            $scope.research()
      ...
      $scope.showCat = (ev, row) => {
        ...
        $scope.refreshing = true
        return $scope.ajax('save_init', {cat_id: row ? row.cat_id : undefined}).then(data => {
          $mdDialog.show({
                                    ...
                                    <form class="md-dialog-content" name="catForm" ng-submit="saveMail()" autocomplete="off">
                                        <div class="input-container">
                                            ...
                                            <md-menu class="md-dropdown cool" md-offset="1 50" >
                                                <md-button ng-click="$mdOpenMenu($event)" class="md-raised md-primary md-hue-1" md-theme="dracular" aria-label="data control" style="max-width: 252px;padding-right: 30px;text-overflow: ellipsis;">
                                                ...
                                                <md-menu-content class="md-dropdown-content cool" style="width: 264px">
                                                    <md-menu-item ng-repeat="item in type_list track by $index"><md-button ng-click="chooseType(item)">{{item.type_name}}</md-button></md-menu-item>
                                                </md-menu-content>
                                            </md-menu> 
                                        </div>
                                        <div class="input-container">
      ...
      let getCheckedQ = () => $scope.ajax('cat_checked').then(data => {
        $scope.check_list = data.data_list
        return data
      }), getUncheckedQ = () => $scope.ajax('cat_unchecked').then(data => {
      ...
      $scope.manageCat = () => {
        ...
        return $q.all([checkQ, uncheckQ]).then(() => {
          $scope.refreshing = false
          return
        })
      }
      $scope.applyCat = () => {
        ...
        if (!list.length) {
          $rootScope.alert('请选择一条分类')
          return
        }

        if ($scope.refreshing) {return}
        $scope.refreshing = true
        return $scope.ajax('cat_apply', {cat_list: list}).then(() => {
          checkQ = getCheckedQ().then(data => {
            $scope.audit_list = $scope.check_list
            return data
          })
