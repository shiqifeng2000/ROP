...
define([], function() {
  'use strict'
  return ['$rootScope', '$scope', '$mdDialog', '$timeout', ($rootScope, $scope, $mdDialog, $timeout) => {
    ...
    $scope.searcher = (index, size) => {
      ...
      if ($scope.refreshing || (pageTotal < pageindex)) {
        $rootScope.nextTick(() => {
          ($scope.pageIndex != $scope.pageIndexClone) && ($scope.pageIndex = $scope.pageIndexClone)
        })
        return
      }
    ...
    $scope.expandRow = row => {
      $scope.tableData && $scope.tableData.length && $scope.tableData.forEach(r => {
        (r !== row) && (r._expand = false)
      })
    ...
    let ParamController = (scope, $mdDialog, data) => {
      scope.data = data
      scope.tooltip = '复制'
      scope.cliper = new Clipboard('.cliper', {
        text: function() {
          return angular.element('#myText')[0].innerText
        }
      })
