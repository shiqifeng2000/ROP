...
define(['../../services'], function (cta) {
  'use strict'
  return ['$rootScope', '$scope', '$q', '$http', '$mdDialog', '$stateParams', '$location',
    ($rootScope, $scope, $q, $http, $mdDialog, $stateParams, $location) => {
      let getUserList = () => {
        return $scope.ajax('list_init').then(data => {
        ...
        return $scope.ajax('list_get', {
          ...
          if (((typeof data.is_success == 'boolean') && data.is_success) || ((typeof data.is_success == 'string') && (data.is_success == 'true'))) {
            ...
            if ($scope.tableData && $scope.tableData[0]) {
              if (previous) {
                $scope.tableData._select = [].concat.apply([], $scope.tableData.map(r => ((previous.app_id == r.app_id) ? [r] : [])))[0]
              } else {
      ...
      // 如果传入参数则自动定位到待办事项上
      if ($stateParams.user_id && $stateParams.app_id) {
        $scope.refreshing = true
        userListQ.then(data => {
          $scope.search_status = "1"
          $scope.userChoosen = [].concat.apply([], $scope.user_list.map(r => (r.user_id == $stateParams.user_id) ? [r] : []))[0]
          return $scope.ajax('api_init', {
            app_id: $stateParams.app_id
          }).then(data => {
            $scope.appSelected = angular.extend({}, data, {app_id: $stateParams.app_id})
            if ($scope.search_status == '1') {
              var firstItem = [].concat.apply([], $scope.appSelected.status_list.map(r => (r.status_id == '0') ? [r] : []))[0]
              $scope.appSelected.statusChoosen = firstItem
            }
        ...
        $rootScope.confirm('确定要审核选中的API吗？', () => {
          ...
          return $http.post('/agent', {
            ...
            if (((typeof body.data.is_success == 'boolean') && body.data.is_success) || ((typeof body.data.is_success == 'string') && (body.data.is_success == 'true'))) {
              $scope.refreshing = false
              if($scope.appSelected && $scope.appSelected.statusChoosen && ($scope.appSelected.statusChoosen.status_id == '0')){
                $scope.apiResearch()
              } else {
                $scope.apiSearcher()
              }
      ...
      $scope.cancelAPI = () => {
        ...
        $rootScope.confirm('确定要解除审核该API吗？', () => {
          $scope.refreshing = true
          return $scope.ajax('api_cancel', {
            app_id: $scope.appSelected ? $scope.appSelected.app_id : '',
            api_id: [].concat.apply([], $scope.apiTableData.map(r => r._checked ? [r.api_id] : []))[0]
          }).then(() => {
            $scope.refreshing = false
            if($scope.appSelected && $scope.appSelected.statusChoosen && ($scope.appSelected.statusChoosen.status_id == '1')){
              $scope.apiResearch()
            } else {
              $scope.apiSearcher()
            }
      ...
      $scope.apiSearcher = (index, size) => {
        if ($scope.refreshing) {return}
        $scope.refreshing = true
        return $scope.ajax('api_get', {
          pageindex: index || $scope.apiPageIndex,
          pagesize: size || $scope.apiPageSize,
          app_id: $scope.appSelected ? $scope.appSelected.app_id : '',
      ...
      let UATController = (scope, uat_list) => {
        ...
        scope.send = () => {
          if (scope.refreshing) {return}
          if (!scope.uat_list._select || !scope.uat_list._select.eid) {
            scope.error = '请选择环境'
            return
          }
          scope.refreshing = true
