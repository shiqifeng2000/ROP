/*define(['clipboard', '../services'], function (Clipboard) {
  'use strict'
...
angular.module('application.services', ['ngMaterial', 'ngCookies', 'rop.services'])
  .factory('HttpObserver', ['$q', '$window', '$cookies', ($q, $window, $cookies) => {
    var sessionInjector = {
      response: function (config) {
  ...
  .factory('LogProcessor', ['$http', '$mdDialog', '$cookies', 'ScopeInitializer', ($http, $mdDialog, $cookies, ScopeInitializer) => {
    let LogController = (scope, $mdDialog, $timeout, $mdUtil, recordkey, api) => {
      let _lang = ScopeInitializer._lang, beforeClipboardCopy = (_lang == 'zh-cn') ? '拷贝信息' : 'Copy',
        afterClipboardCopy = (_lang == 'zh-cn') ? '已复制' : 'Copied',
        workaroundSupportClipboard = action => {
          let actionMsg = ` 来${action === 'cut' ? '剪切' : '拷贝'}`, actionKey = (action === 'cut' ? 'X' : 'C')
    ...
    return {
      showLog (ev, recordkey, api) {
        $mdDialog.show({
          ...
          parent: angular.element(document.querySelector('body>section>md-content')),
          clickOutsideToClose: true,
          locals: {recordkey: recordkey, api: api}
        })
      }
    }
  }])
