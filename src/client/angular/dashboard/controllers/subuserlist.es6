...
define(['../../services'], function() {
  ...
  return ['$rootScope', '$scope', '$q', '$http', '$mdDialog', '$location',
    ($rootScope, $scope, $q, $http, $mdDialog, $location) => {
      $scope.columns = [
        { text: '用户账号', name: 'user_account', style: { width: '160px', 'text-align': 'center' } },
        { text: '用户昵称', name: 'user_name', style: { width: '160px', 'text-align': 'center' } },
        { text: '公司名称', name: 'company', style: { width: '160px', 'text-align': 'center' } },
...
子账号登录时，登录规则为“主账号登录名称:子账号用户名称”，例如：“ruixue_isv:laurence”。
      ...
      $scope.showAppList = () => {
        ...
        $scope.mode = 2
        $scope.refreshing = true
        $rootScope.ajax('app_get', {
          sub_user_id: $scope.tableData._select.sub_user_id
        }).then((data) => {
          if (((typeof data.is_success == 'boolean') && data.is_success) || ((typeof data.is_success == 'string') && (data.is_success == 'true'))) {
            $scope.appList = data.data_list
          }
        }).finally(() => {
          $scope.refreshing = false
        })
      ...
      $scope.checkApp = () => {
        let value = $scope.appList._checked
        $scope.appList.forEach(app => {
          app.role_flag = value
        })
      }
