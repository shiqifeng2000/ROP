...
define(['../../services'], function() {
  ...
  return ['$rootScope', '$scope', '$q', '$http', '$mdDialog',
    ($rootScope, $scope, $q, $http, $mdDialog) => {
      ...
      $scope.save = () => {
        if ($scope.loading || $scope.refreshing) { return }
        if ($scope.mailForm.$invalid || $scope.smsForm.$invalid || $scope.urlForm.$invalid) {
          ...
          if ($scope.smsForm.$invalid) {
            $scope.smsForm.$error.required && $scope.smsForm.$error.required.forEach(r => { r.$setDirty(true) })
            $scope.smsForm.$error.maxlength && $scope.smsForm.$error.maxlength.forEach(r => { r.$setDirty(true) })
            $scope.smsForm.$error.pattern && $scope.smsForm.$error.pattern.forEach(r => { r.$setDirty(true) })
          }
