    ...
    return ['$rootScope', '$scope', '$q', '$location', '$http', '$timeout', '$mdDialog', 'MiscTool',
      ($rootScope, $scope, $q, $location, $http, $timeout, $mdDialog, MiscTool) => {
        ...
        $scope.selectMode = mode => {
          ...
          if (mode == 0) {
            apiQ.then(() => {$scope.mode = mode}, () => {$scope.mode = mode})
          } else if (mode == 1) {
            sysErrorQ.then(() => {$scope.mode = mode}, () => {$scope.mode = mode})
          } else if (mode == 2) {
            urlErrorQ.then(() => {$scope.mode = mode}, () => {$scope.mode = mode})
          }
        }
        $scope.onOpen = ()=>{
          console.log("i show")
        ...
        $scope.urlTimeout = ''
        // mode 0 情况
        // 对于app应用没有提供不分页的接口，只好用一个很大的数做pagesize
        // 共通
        let getMailQ = () => $scope.ajax('mail_list').then(data => {
          ...
          getMobileQ = () => $scope.ajax('mobile_list').then(data => {
          ...
          appQ = $scope.ajax('app_list', {pageindex: 1, pagesize: 9999}).then(data => {
            $scope.app_list = data.app_list
            if (!$scope.app_list.length) {
              return $q.reject('无任何应用的情况下，该页面不可用')
            }
            $scope.app_list._error_selected = $scope.app_list[0]
            $scope.app_list._sys_selected = $scope.app_list[0]
            return data
          }),
          ...
          getAPIQ = () => {
            $scope.loading = true
            $scope.api_list = []
            return appQ.then(() => $scope.app_list._error_selected ? $scope.ajax('api_list', {app_id: $scope.app_list._error_selected.app_id}) : $q.reject()).then(data => {
          ...
          getAPIErrorQ = () => {
            $scope.api_error = []
            $scope.api_error_all = []
            $scope.getAPIErrorAll = () => []
            return getAPIQ().then(() => ($scope.api_list[0] ? $scope.selectAPI($scope.api_list[0]) : $q.reject()))
          },

          apiQ = getAPIErrorQ(),
          //apiErrorQ = apiQ.then(getAPIErrorQ),

          // 业务级 系统 错误#
          getSysError = () => $scope.ajax('sys_error_list', {app_id: $scope.app_list._sys_selected.app_id}).then(data => {
            $scope.sys_error = data
          }),
          sysErrorQ = appQ.then(() => $scope.app_list._sys_selected ? getSysError() : $q.reject()),

          // 业务级 网页 错误
          getUrlError = () => $scope.ajax('url_list').then(data => {
            $scope.url_error = data
          }),
          urlErrorQ = getUrlError()

        $scope.refreshing = true
        $q.when(apiQ, sysErrorQ, urlErrorQ).finally(() => {$scope.refreshing = false})
        ...
        $scope.selectApp = app => {
          ...
          $scope.refreshing = true
          if ($scope.mode == 0) {
            $scope.app_list._error_selected = app
            apiQ = getAPIErrorQ().finally(() => {$scope.refreshing = false})
          } else if ($scope.mode == 1) {
            $scope.app_list._sys_selected = app
        ...
        $scope.reset = () => {
          ...
          if ($scope.mode == 0) {
            $scope.apiKeyword = ''
            $scope.app_list._error_selected = $scope.app_list[0] || undefined
            apiQ = getAPIErrorQ().finally(() => {$scope.refreshing = false})
          } else if ($scope.mode == 1) {
            $scope.sysKeyword = ''
            $scope.app_list._sys_selected = $scope.app_list[0] || undefined
            sysErrorQ = getSysError().finally(() => {$scope.refreshing = false})
        ...
        $scope.selectAPI = api => {
          if ($scope.api_list && $scope.app_list && $scope.app_list._error_selected) {
            $scope.api_list._selected = api
            $scope.refreshing = true
            return $scope.ajax('error_list', {
              api_id: api.api_id,
              app_id: $scope.app_list._error_selected.app_id
            }).then(data => {
        ...
        $scope.apiErrorSave = () => apiQ.then(() => {
          ...
          if ($scope.app_list && $scope.app_list._error_selected && $scope.api_list) {
            $scope.refreshing = true
        ...
        $scope.sysErrorSave = () => sysErrorQ.then(() => {
          ...
          if ($scope.app_list && $scope.app_list._sys_selected && $scope.sys_error && $scope.sys_error.api_list && $scope.sys_error.api_list.length && $scope.sys_error.error_list && $scope.sys_error.error_list.length) {
            $scope.refreshing = true
            return $scope.ajax('sys_error_save', {
              app_id: $scope.app_list._sys_selected.app_id,
              api_list: list,
        ...
        let MailController = (scope, mail_list) => {
          ...
          scope.saveMail = () => {
            ...
            var mail_error_list_length = scope.mail_list && scope.mail_list.length || 0,
              uniqueList = [...new Set((scope.mail_list || []).map(r => r.user_mail))]
                                            ...
                                            <form name="mailForm" class="table-wrapper" autocomplete="off">
                                                ...
                                                <div class="scroller" id="{{uniqueID}}">
                                                    <table cellspacing="0" class="md-datatable">
                                                        <tbody>
                                                        <tr ng-repeat="mail in mail_list track by $index">
                                                            <td>
        ...
        let MobileController = (scope, mobile_list) => {
          ...
          scope.saveMobile = () => {
            ...
            var mobile_error_list_length = scope.mobile_list && scope.mobile_list.length || 0,
              uniqueList = [...new Set((scope.mobile_list || []).map(r => r.user_mobile))]
                                        ...
                                        <div class="md-dialog-content" >
                                            <form name="mobileForm" class="table-wrapper" autocomplete="off">
                                                ...
                                                <div class="scroller" id="{{uniqueID}}">
                                                    <table cellspacing="0" class="md-datatable">
                                                        <tbody>
                                                        <tr ng-repeat="mobile in mobile_list track by $index">
                                                            <td>
