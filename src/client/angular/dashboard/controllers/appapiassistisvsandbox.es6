      ...
      ($rootScope, $scope, $q, $location, $mdDialog, ROPDateUtil) => {
        ...
        $scope.listAPI = () => {
          ...
          return $scope.ajax('log_get', {
            ...
            $scope.propertyName = 'content'
            $scope.sortBy($scope.propertyName, true)
            $scope.log_summary = {
              callcount: 0,
              successcount: 0,
              failcount: 0,
        ...
        $scope.realtimeAPIs = () => {
          ...
          return $scope.ajax('realtime_apis', {
            search_begin: today,
            search_end: today,
        ...
        $scope.enterDetail = log => {
          ...
          $scope.mode.log = log
          $scope.mode.isGrid = $scope.mode.isGrid === undefined ? true : $scope.mode.isGrid
            ...
            let logSlicer = log_list => {
              ...
              if (scope.mode == 2) {
                ...
                var maxDayIndex = scope.maxdate.getDay(),
                  firstDayOfTheMaxDateWeek = ROPDateUtil.incrementDays(scope.maxdate, 1 - (maxDayIndex || 7)),
                  minDayIndex = scope.mindate.getDay(),
                  firstDayOfTheMinDateWeek = ROPDateUtil.incrementDays(scope.mindate, 1 - (minDayIndex || 7)),
                ...
                log_list.forEach((r, i) => {
                  ...
                  if ((scope.mindate <= targetDate) && (targetDate <= lastDayOfTheMinDateWeek)) {
                    ...
                    var dayIndex = targetDate.getDay(),
                      firstDayOfTheWeek = ROPDateUtil.incrementDays(targetDate, 1 - (dayIndex || 7)),
        ...
        $scope.showCountCall().then(data => {
          $scope.summary.countCalls = data.list_count
        })

        $scope.sortBy = (propertyName, reverse) => {
          $scope.reverse = reverse !== undefined ? reverse : (($scope.propertyName === propertyName) ? !$scope.reverse : false)  // true: 正序  false: 倒序
          $scope.propertyName = propertyName
          $scope.log_list.sort(function(a, b){
            let nameA = ''
            let nameB = ''
            if (a[$scope.propertyName] === undefined || b[$scope.propertyName] === undefined) {
              return 0
            } else if (propertyName === 'content' && $scope.mode.id === 1) {
              nameA = a[$scope.propertyName].toUpperCase()
              nameB = b[$scope.propertyName].toUpperCase()
            } else if (propertyName === 'content' && $scope.mode.id === 2) {
              nameA = a[$scope.propertyName]
              nameB = b[$scope.propertyName]
              let arrA = nameA.split('.')
              let arrB = nameB.split('.')
              if (nameA === nameB || arrA.length != 4 || arrB.length != 4){
                return 0
              } else {
                for (let i = 0; i < 4; i++) {
                  if (Number.parseInt(arrA[i]) < Number.parseInt(arrB[i])){
                    return $scope.reverse ? -1 : 1
                  } else if (Number.parseInt(arrA[i]) > Number.parseInt(arrB[i])) {
                    return $scope.reverse ? 1 : -1
                  }
                }
              }
            } else if (propertyName === 'content' && $scope.mode.id === 3) {
              let strA = a[$scope.propertyName].toUpperCase()
              let strB = b[$scope.propertyName].toUpperCase()
              nameA = Number.parseFloat(strA.slice(-2, strA.length))
              nameB = Number.parseFloat(strB.slice(-2, strB.length))
            } else if (!Number.isNaN(Number.parseFloat(a[$scope.propertyName]))) {
              nameA = Number.parseFloat(a[$scope.propertyName])
              nameB = Number.parseFloat(b[$scope.propertyName])
            }
            if (nameA < nameB) {
              return $scope.reverse ? -1 : 1
            } else if (nameA > nameB) {
