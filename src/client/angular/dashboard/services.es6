/*define(require => {
 ...
 return angular.module('dashboard.services', [])
...
angular.module('dashboard.services', ['ngMaterial', 'ngCookies', 'rop.services'])
  .factory('HttpObserver', ['$q', '$window', '$cookies', ($q, $window, $cookies) => {
    var sessionInjector = {
      response: function (config) {
        if (config.data && config.data._expired) {
  ...
  .factory('LogProcessor', ['$http', '$mdDialog', '$cookies', 'ScopeInitializer', ($http, $mdDialog, $cookies, ScopeInitializer) => {
    let LogController = (scope, $mdDialog, $timeout, $mdUtil, recordkey, api, title) => {
      let _lang = ScopeInitializer._lang, beforeClipboardCopy = (_lang == 'zh-cn') ? '拷贝信息' : 'Copy',
      ...
      scope.title = title === undefined ? '操作日志' : title
      scope.reset = e => {
        scope.columns = [{
          text: '操作时间',
          name: 'create_time',
    ...
    return {
      showLog (ev, recordkey, api, title) {
        $mdDialog.show({
          ...
          locals: {recordkey: recordkey, api: api, title: title}
        })
      }
    }
  }])
    ...
    return {
      append (list, scrollerSelector) {
        ...
        if (scrollerSelector) {
          ...
          $el && $el.animate({scrollTop: $el[0].scrollHeight})
        }
      },
      splice (list, target, scrollerSelector) {
        list.splice(list.indexOf(target), 1)
        if (scrollerSelector) {
          let $el = angular.element(scrollerSelector)
          $el && $el.animate({scrollTop: 0})
        }
      }
    }
  }])
