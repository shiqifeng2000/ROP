...
router.use('/:func-:id.html', function (request, response, next) {
    ...
    if (func == "ApiMethod" || func == "ApiDomain") {
        response.redirect(constant.protocol + '://' + constant.domain + ':' + constant.port + '/supplier/document/' + (func == "ApiMethod" ? 'api' : 'domain') + '/' + id);
        return;
    }
    param[func == "ApiPreview" ? "api_id" : "domain_id"] = id;
...
/*router.get('/ApiMethod-:id.html', (request, response) => {
    ...
    let param = {_session_id: request.session.id,_info:JSON.stringify(info),_injection: constant.clientSideScript};
...
router.get('/ApiPreview-:id.html', function (request, response) {
    info.method = "api";
    info.id = request.params.id;
    var param = { _info: JSON.stringify(info), _injection: constant.clientSideScript };
