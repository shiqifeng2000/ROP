...
var managePost = function managePost(request, response) {
  ...
  requester.post({ url: headers.origin + (path ? '/' + path : ''), form: param, headers: headers }).pipe(response, { end: true });
...
var manageGet = function manageGet(request, response) {
  ...
  headers.host = constant.legacyDomain; //"115.159.67.217:30005";
  requester({
    url: headers.origin + (path ? '/' + path : '') + (!request._isSubstitue && request.session.profile ? '?session_id=' + request.session.profile.login_user_id : ''),
